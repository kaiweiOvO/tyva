...
export default function vaildateType<T>(value: T, type: ValueType<T>): void {
  if (typeof value === "object" && Array.isArray(value) === false) {
    ...
    if (typeof type === "string") {
      ...
      // TypeOptions
      checkTypeOption(value, type as TypeOptions<typeof value>);
    }
  }
}
...
function checkType(value: unknown, type: Type, key?: string) {
  if (type === "array" && Array.isArray(value) === false) {
    // value === array
    throw new Error(
      `${key ? `parameter ${key} ` : ""}expected type is ${type}, but received ${typeof value}.`
    );
  } else if (Array.isArray(value) === true && type !== "array") {
    // value === string | number | bool | object | undefined
    throw new Error(
      `${key ? `parameter ${key} ` : ""}expected type is ${type}, but received array.`
    );
  ...
  switch (option.type) {
    ...
    case "undefined":
      ...
      if (typeof value !== option.type) {
        throw new Error(
          `${key ? `parameter ${key} ` : ""}expected type is ${
            option.type
...
function validateString(
  ...
  if (length !== undefined && value.length !== length) {
    throw new Error(
      `${key ? `parameter ${key} ` : ""}expected length ${length}, got value length ${
        value.length
      }.`
    );
  }
  if (maxLen !== undefined && value.length > maxLen) {
    throw new Error(
      `${key ? `parameter ${key} ` : ""}expected maximum length ${maxLen}, got value length ${
        value.length
      }.`
    );
  }
  if (minLen !== undefined && value.length < minLen) {
    throw new Error(
      `${key ? `parameter ${key} ` : ""}expected minimum length ${minLen}, got value length ${
        value.length
      }.`
    );
  }
  if (format !== undefined && format.test(value) === false) {
    throw new Error(
      `${key ? `parameter ${key} ` : ""}expected format ${format}, got mismatched value ${value}.`
    );
  }
}
...
function validateNumber(value: number, float?: boolean, max?: number, min?: number, key?: string) {
  ...
  if (max !== undefined && value > max) {
    throw new Error(
      `${key ? `parameter ${key} ` : ""}value ${value} is bigger than maximum value ${max}.`
    );
  }
  if (min !== undefined && value < min) {
    throw new Error(
      `${key ? `parameter ${key} ` : ""}value ${value} is smaller than minimum value ${min}.`
    );
  }
}
