...
describe("validateType", () => {
  ...
  it("validate object", () => {
    ...
    expect(() =>
      validateType({ name: "test" }, { name: { type: "string", length: 2 } })
    ).toThrowError(new Error("parameter name expected length 2, got value length 4."));
    expect(() =>
      validateType({ name: "test" }, { name: { type: "string", maxLen: 2 } })
    ).toThrowError(new Error("parameter name expected maximum length 2, got value length 4."));
    expect(() =>
      validateType({ name: "test" }, { name: { type: "string", minLen: 5 } })
    ).toThrowError(new Error("parameter name expected minimum length 5, got value length 4."));
    expect(() =>
      validateType({ name: "test" }, { name: { type: "string", format: /d+/ } })
    ).toThrowError(new Error("parameter name expected format /d+/, got mismatched value test."));
    ...
    expect(() =>
      validateType({ age: 18 }, { age: { type: "number", min: 16 } })
    ).not.toThrowError();
    ...
    expect(() =>
      validateType({ devices: [1] }, { devices: { type: "array", length: 1 } })
    ).not.toThrowError();
    expect(() =>
      validateType({ devices: [1] }, { devices: { type: "array", length: 2 } })
    ).toThrowError(new Error("parameter devices expected length 2, got value length 1."));
    expect(() =>
      validateType({ devices: [1] }, { devices: { type: "array", each: "number" } })
    ).not.toThrowError();
    expect(() =>
    ...
    expect(() =>
      validateType({ devices: [1] }, { devices: { type: "array", maxLen: 1 } })
    ).not.toThrowError();
    expect(() =>
      validateType({ devices: [1, 2] }, { devices: { type: "array", maxLen: 1 } })
    ).toThrowError(new Error("parameter devices expected maximum length 1, got value length 2."));
    expect(() =>
      validateType({ devices: [1] }, { devices: { type: "array", minLen: 1 } })
    ).not.toThrowError();
    expect(() =>
      validateType({ devices: [1] }, { devices: { type: "array", minLen: 2 } })
    ).toThrowError(new Error("parameter devices expected minimum length 2, got value length 1."));
  });
  it("validate array", () => {
    expect(() => validateType(1, "array")).toThrowError(
      new Error("expected type is array, but received number.")
    );
